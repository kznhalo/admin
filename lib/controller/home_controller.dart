...
import 'package:kozarni_ecome/model/auth_user.dart';
...
import '../model/order_revenue.dart';
...
class HomeController extends GetxController {
  ...
  final Rx<AdminUser> user = AdminUser().obs;
  ...
  Future<void> confirm() async {
    try {
      await _auth
          .loginWithCerdential(PhoneAuthProvider.credential(
            verificationId: _codeSentId.value,
            smsCode: verificationController.text,
          ))
          .then((value) => addingAdmin(phoneNumber: phoneController.text));
      _codeSentId.value = '';
  ...
  void onInit() async {
    ...
    _auth.onAuthChange().listen((_) async {
      user.value = AdminUser(user: _);
      if (_ == null) {
        authorized.value = false;
      } else {
        //if User is not null,we get IdToken
        _auth.gettokenResult().then((value) {
          if (value.claims?["isAdminUser"] == true) {
  ...
  RxList<AuthUser> authUserList = <AuthUser>[].obs;
  ...
  Future<void> getAuthUsers() async {
    ...
    try {
      ...
      callable.call().then((value) {
        final decodeJson = jsonDecode(value.data) as List<dynamic>;
        if (decodeJson.isNotEmpty) {
          debugPrint("**************response: ${decodeJson}");
          final list = decodeJson.map((e) => AuthUser.fromJson(e)).toList();
          authUserList.value = list
              .where((element) => element.customClaims?.isPartnerUser == true)
              .toList();
        }
        isPageLoading.value = false;
      });
    } catch (e) {
      debugPrint("Error Try Getting Auth Users");
      isPageLoading.value = false;
    }
  }

  Future<void> addingAdmin({required String phoneNumber}) async {
    try {
      //Phone login complete,we call our admin cloud function
      HttpsCallable callable =
          FirebaseFunctions.instanceFor(region: "asia-southeast2")
              .httpsCallable('createAdminUserWithPhoneNumber');
      final response = await callable.call(<String, dynamic>{
        'phone': phoneNumber,
      });
      debugPrint("*****************AdminResponse: ${response.data}");
